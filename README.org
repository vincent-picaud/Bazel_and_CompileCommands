...
*** The =setup_compile_commands.sh= script
    ...
    copies and generates all the required files (see [[Files_2_Create][Directories
    and files creation]]).
...
*** The =create_compile_commands.sh= script
 ...
 Choose a =Bazel= target *A_BAZEL_TARGET*, like =...= or =//my:target= and run:
...
This section describes the content of the two Shell scripts.
